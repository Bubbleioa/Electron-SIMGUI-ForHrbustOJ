        ...
        .then((result) => {
          if (!result.canceled) {
            ...
            fs.readFile(result.filePaths[0], async (err, data) => {
              ...
              this.$message({
                message: `共转换出${res.length}份代码，请选择下一步。`,
                type: "success",
                showClose: true,
                duration: 2000,
              });
        ...
        .then((result) => {
          if (!result.canceled) {
            ...
            if (re.test(result.filePaths[0])) {
              ...
              fs.readdirSync(result.filePaths[0]).forEach(function (
                item,
                index
              ) {
              ...
              if (allFileSize >= 0.001) {
                message =
                  "共选择 " +
                  fileTableData.tableData.length +
                  "个文件，总大小为 " +
                  allFileSize.toFixed(3) +
                  " MB";
              } else {
                message =
                  "选择文件夹成功，" +
...
var doneStep1Vue = new Vue({
  ...
  methods: {
    next() {
      ...
      if (stepBarVue.active++ >= 3) stepBarVue.active = 0;
      if (stepBarVue.active == 0) {
        ...
        chooseSourceFileData.isShow = true;
        chooseSourceFileData.filePath = "";
        chooseSourceFileData.disabled = false;
      ...
      if (stepBarVue.active == 1) {
        ...
        chooseFilePathData.filePath = "";
        chooseFilePathData.isShow = true;
        chooseSourceFileData.isShow = false;
        chooseSourceFileData.disabled = true;
        doneStep1Data.buttonText = "下一步";
      }
      if (stepBarVue.active == 2) {
        chooseFilePathData.disabled = true;
